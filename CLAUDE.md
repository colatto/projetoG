...
> - **AC-06 (resiliência §9.6):** `AuditService` enfileira via pg-boss (`audit:retry`, retryLimit: 3, retryDelay: 60s) quando o insert em `audit_logs` falha. Fallback é best-effort — se pg-boss também falhar, o erro é logado mas nunca bloqueia o caller. **Hardening (2026-05-03):** `try/catch` externo adicionado em `registerEvent()` para capturar exceções lançadas pelo client Supabase (e.g., `Promise.reject`, erros de rede) além de respostas `{ error }` — corrige falha no teste de resiliência do dashboard (`dashboard.routes.test.ts`).
