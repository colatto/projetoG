...
async function run(): Promise<void> {
  ...
  const supplierEntries: { supplierId: number; supplierName: string }[] = [];
  ...
  for (const entry of supplierEntries) {
    try {
      const creditor = await client.get<CreditorResponse>(`/creditors/${entry.supplierId}`, {
        correlationId: `v5-${entry.supplierId}`,
        source: 'unknown',
      });
  ...
  for (const r of results) {
    ...
    const nameCheck = r.found
      ? r.supplierName?.toUpperCase().trim() === r.creditorName?.toUpperCase().trim()
        ? '🔗 name match'
        : `🔗 name differs: "${r.creditorName}"`
      : '';
    ...
    if (r.found) {
      ...
      console.log(
        `      supplier flag: ${r.isSupplier} | active: ${r.isActive} | contacts: ${r.contactCount}`,
      );
      console.log(`      ${email}`);
  ...
  console.log(
    `   Resolved as creditor:          ${found.length}/${results.length} (${Math.round((found.length / results.length) * 100)}%)`,
  );
  ...
  if (notFound.length === 0) {
    ...
    console.log(
      `   ⚠️ MOSTLY CONFIRMED: ${found.length}/${results.length} resolved (${Math.round((found.length / results.length) * 100)}%)`,
    );
    console.log(
      `      ${notFound.length} ID(s) did not resolve: ${notFound.map((r) => r.supplierId).join(', ')}`,
    );
  ...
  if (found.length === 0) {
    ...
    console.log(
      `   ⚠️ ${withoutEmail.length}/${found.length} creditors (${pct}%) have NO email in contacts[].`,
    );
    console.log('      Per PRDGlobal §9.5, these suppliers would be blocked.');
    console.log('      Affected:');
    for (const r of withoutEmail) {
      console.log(
        `        - ID ${r.supplierId}: ${r.supplierName} (${r.contactCount} contacts, none with email)`,
      );
    }
  ...
  console.log(
    '      The supplierId is at result.suppliers[].supplierId, NOT at result.supplierId.',
  );
