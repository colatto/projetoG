...
function runWasmBundle() {
  ...
  if (!cli) {
    console.error(
      '[build-hostinger-api] esbuild-wasm is not installed. Add esbuild-wasm to the workspace root devDependencies.',
    );
    process.exit(1);
  }
  console.warn(
    '[build-hostinger-api] Using esbuild-wasm (WebAssembly). Suitable when the host denies executing native binaries (e.g. noexec under public_html on shared hosting).',
  );
...
function main() {
  ...
  if (!bin) {
    console.warn('[build-hostinger-api] Native esbuild binary not found; using esbuild-wasm.');
    runWasmBundle();
    writeDistPackageJson();
    return;
  }
