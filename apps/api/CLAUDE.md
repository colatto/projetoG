...
- `AuditService` centralizado (`audit.service.ts`): `registerEvent()` com `summary` obrigatório (fallback automático via `fallbackSummary()`), campos operacionais PRD-09 (`actorType`, `purchaseOrderId`, `supplierId`), e enfileiramento pg-boss (`audit:retry`) em caso de falha de escrita (§9.6). Todos os módulos (orders, dashboard, quotations, damages, followup) usam exclusivamente `AuditService` — zero inserts diretos em `audit_logs`.
