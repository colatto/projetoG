...
- aliases de compatibilidade PRD-09 para rotas de cotação e para pedidos (`/api/backoffice/orders` espelha `/api/orders`)
...
| orders          | `/api/orders`, `/api/backoffice/orders`                       | `GET` lista com `require_action`, `sort_priority` (PRD-09), ordenação RN-09 + contexto `follow_up_trackers`; entregas, cancelamento, histórico (PRD-05)                                                                       |
...
| dashboard       | `/api/dashboard`                                              | resumo, kpis, lead-time, atrasos, criticidade, ranking, avarias (PRD-08); filtros globais fornecedor/obra/pedido/item em lead-time, atrasos, criticidade, ranking e avarias; `dashboard.access` em `audit_logs` (best-effort) |
...
- testes: 168 passando — inclui `dashboard.routes.test.ts` (12 cenários: RBAC, ranking, criticidade, **resumo**, **audit `dashboard.access`**, **atrasos + PO**, **ranking filtrado**, **criticidade 400**, **audit resiliente**), `notification.routes.test.ts` (21 cenários PRD-03 §7.5), `followup.routes.test.ts`, `damages.routes.test.ts` (PRD-06 §6.1/§14), **`webhooks.test.ts`** (pipeline ACK-only `CONTRACT_AUTHORIZED` → `related_entity_type: contract`), **`integration.test.ts`** (filtros `direction` + `date_from`/`date_to`)
- build: passa
- lint: passa
...
- Paginação e filtros `supplier_id` / `building_id` em `GET /api/orders` conforme campos mínimos §14.1 do PRD-09 (evolução futura).
