...
export class UsersController {
  ...
  async createUser(
    ...
    if (payload.role === UserRole.FORNECEDOR && payload.supplier_id) {
      ...
      if (supplierConflict) {
        return reply.code(400).send({ message: 'O ID de Fornecedor informado já possui um acesso ativo' });
      }
    }

    // Supabase Auth Creation
    // Utilizing admin API to invite user, it generates user and emails the magic link
    const { data: authUser, error: authError } = await supabase.auth.admin.inviteUserByEmail(payload.email);
  ...
  async blockUser(
    ...
    if (user.status === UserStatus.BLOQUEADO) return reply.code(409).send({ message: 'Usuário já está bloqueado' });
    if (user.status === UserStatus.REMOVIDO) return reply.code(409).send({ message: 'Não é possível operar sobre usuário removido' });
  ...
  async reactivateUser(
    ...
    if (!user) return reply.code(404).send({ message: 'Usuário não encontrado' });
    if (user.status !== UserStatus.BLOQUEADO) return reply.code(409).send({ message: 'Usuário não está bloqueado' });
  ...
  async deleteUser(
    ...
    if (!user) return reply.code(404).send({ message: 'Usuário não encontrado' });
    if (user.status === UserStatus.REMOVIDO) return reply.code(409).send({ message: 'Usuário já está removido' });
  ...
  async resetPasswordByAdmin(
    ...
    if (!user) return reply.code(404).send({ message: 'Usuário não encontrado' });
    if (user.status === UserStatus.REMOVIDO) return reply.code(403).send({ message: 'Usuário removido' });
