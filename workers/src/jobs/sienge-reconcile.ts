...
export async function processSiengeReconcile(job: PgBoss.Job): Promise<void> {
  ...
  try {
    ...
    await supabase.from('integration_events').insert({
      ...
      http_status: 200,
      status: 'success',
      request_payload: sanitizeForLog({ entityType, entityId, reason }),
      response_payload: sanitizeForLog(result.summary),
...
async function registerReconciliationDivergence(
  ...
  await supabase.from('integration_events').insert({
    ...
    response_payload: sanitizeForLog({ divergences: result.divergenceMessages }),
    related_entity_type: result.relatedEntityType,
    related_entity_id: String(result.entityId),
    retry_count: 0,
    max_retries: 0,
